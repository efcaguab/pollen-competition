...
loadd(plant_rel_abu, plant_pheno_overlap, degree, sites)
...
tidied_fixed %>%
  separate("level", c("plant_name", "site_name"), sep = "\\.", remove = F) %>%
  filter(term == "(Intercept)" ,
         var_trans == 'log',
         scale == "community", 
...
summ <- glanced_random %>%
  ...
  mutate(delta_AIC = paste0(AIC_50, " [", AIC_05, ", ", AIC_95, "], n = ", n), 
         delta_AIC = if_else(grepl("NA", delta_AIC), "---", delta_AIC)) %>%
  group_by() %>%
  humanize(random_effects = random_effects) %>%
  arrange(pollen_category, random_name, scale) %>%
...
wilcox_glo_com %>%
  filter(quality %in% c("rmse", "sigma", "r2c", "o2"),
         var_trans == "log") %>%
  ...
  rename(`model metric` = quality,
         ...
         `p value` = p.value) %>% 
  xtable(digits = -1, caption = "Results of two sample paired Wilcoxon signed rank test comparing the root mean square error (rmse), the residual standard deviation (sigma), the conditional r-squared approximation as proposed by Nakagawa and Schielzeth (2013), and the omega-squared value as suggested by Xu (2003) of the model sets constructed using predictors computed at the community or study level.", 
