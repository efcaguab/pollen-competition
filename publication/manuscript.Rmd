...
All together, the few existing studies show that, as expected, the effective result of mutualistic interactions is, in most cases, one of facilitation. 
...
But understanding what makes species and communities different is important not only because the ecological and evolutionary implications. 
...
An important proportion of widely used theoretical models of pollination account for the negative effects of mutualisim by shaping the benefits
...
# loadd(model_correlations)

# p1 <- model_correlations %>%
#   filter(var_trans == "log", 
#          scale == "global") %>%
#   ggplot(aes(x = pearson_slope)) + 
#   geom_vline(xintercept = 0.5, size = 0.25, linetype = 2) +
#   stat_density(trim = F, geom = "line") +
#   pub_theme() +
#   labs(x = "slope\n(heterospecific ~ conspecific pollen gain)")
# 
get_pred_range <- function(x){
  tidied_fixed %>% filter(pollen_category == x, var_trans == "log", scale == "global") %$% estimate %>% range()
}
...
p1 <- data_frame(x = get_pred_range("heterospecific"),
  ...
  pub_theme() +
  labs(x = "gain in heterospecific pollen", y = "gain in conspecific pollen")
...
cowplot::plot_grid(p1, p2, align = "h", axis = "tb", ncol = 2, labels = LETTERS[1:2], label_size = 8, rel_widths = c(0.35, 0.65))
```
