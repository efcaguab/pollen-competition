...
Over all study sites, we found that `r round(prop_sig_global*100)`% of species observed a significant increasse (at the $\alpha = 0.05$ level) in pollen density when comparing open and bagged flowers (Table S1). 
When looking at the differences within sites, we found that only `r round(prop_consistency*100)`% of the species that were present in more than one site had the same response across community (Table S2; Figure S1 and S2). 
Despite these differences, the community had a comparatively small effect on wether animal pollination increased the pollen density or not (Table S3).
