...
We used a PCA of the analysed species to investigate whether plant's niche of competition for pollination are similar across communities or whether they are flexible, and therefore a reflection of the community context. 
...
At the same time, receiving high visitation increases the absolute levels of conspecific pollen deposition but increasses heterospeicific deposition to a greater extent---even when the visitors are likely carry a high proportion of conspecific pollen. 
...
However, over long periods of time there could be a risk associated to a specialist plant having no pollinators. 
...
One possible solution is to share pollinators *and* have original traits---as we show that trait originallity is generally benefitial to pollen deposition and it’s commonly thought that species that are further from others in trait space suffer reduced competition. 
...
Second, the usuallly negative, relationship between originality and generalism has been shown depend on the plants abundance [@coux_linking_2016], with generalist species being able to also have original traits only when they are abundant enough to provide a valuable reward to make it worth to pollinators. 
...
Our results highlight the difficulties involved with predicting wether the introduced plant species will facilitate or compete with neighbours. 
...
However, our results also suggest that introduced species are more likely to be benefitial if they have differing traits to local plants. 
...
Instead, we confirm that plant competition is pervasive in pollination communities and that potential strategies to minimise competition are likely involve strong trade-offs in the pollination service either in the short or the long term. 
As expected, not sharing pollinators and being functionally unique reduces competition for pollination by increassing conspecific pollen deposition (both in absolute numbers and relative to heterospecific pollen). 
However, this increasse is surprisingly small when compared to the effect of relative abundance and visitation. 
...
Species coexistence is not solely determined by direct and indirect mutualistic relationships, but also strongly shaped by the multiple intewined ecological factors that shape competition for pollination at the community level. 
Achieving a better understanding how biodiversity is supported by pollination communities will require seeing them as both mutualistic and competitive comminities. 
