...
loadd(abs_cordcount, msc_wordcount, n_displays, n_references)
```
...
Indeed, we found that the proportion of species that experienced a statistically significant positive or neggative relationship between heterospecific and conspecific pollen than neggative was very similar (`r counts_facilitation_slope$np[3]`% and `r counts_facilitation_slope$np[1]`%, respectively; Figure \@ref(fig:fig-trade-off-predictions)a; Table \@ref(S-tab:table-facilitation-models-random)). 
...
In particular, we found that a considerable proportion of species experiences overall neutral or competitive effects from animal-mediated pollination. 
...
Instead, we observed a negative relationships between conspecific and heterospecific in a considerable proportion of the analysed species. 
This relationship could arise, for example, when another, more dominant species, monopolizes a limited number of pollinators [@mitchell_new_2009; @lopezaraizamikel_impact_2007], when heterospecific pollen deposited in the stigma phisically interferes with deposition of conspecific pollen [@runquist_pollinator-mediated_2012], or when conspecific pollen is lost to heterospecific stigmas [@mitchell_new_2009; @muchhala_interspecific_2012]. 
Similarly, not all species had more pollen deposited when open to animal pollination compared to when they were closed. 
Although it can be important, we were unable to test the effect of pollen viability or the extent to which self- vs. outcross-pollen may differ in their value for fertilization (TODO: ref needed). 
However, it is reasonable to expect that the difference between these two may be generally relevant. 
Particularly so for species that are not self-fertile or those in which self-fertilization is rarely effective due to a temporary separation in the maturation of the sexes (dichogamy) (TODO: ref needed). 
These findings agree with a recent study showing a wide range of outcomes in a multi-species context [@tur_evaluating_2016]. 
