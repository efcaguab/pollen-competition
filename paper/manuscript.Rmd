...
We collected data from eleven coflowering plant communities and their pollinators distributed across three locations, each in a distinct reagion of the Argentinian Pampas. 
These locations are distributed across a gradient of precipitation with a humid temperate climate in the eastward Flooding Pampas and a climate drier as we move inland and westward towards the Pampean Grasslands.
...
This subset comprised between three and nine common entomophilus species that were flowering during the sampling period. 
We chosed plant species such that they cover a wide range on a specialization-generalization gradient and a wide range of abundances. 
...
Our second objetive is to investigate the relative contribution that different factors that describe the plants' role and its ecological context have on the pollination service. 
...
Here, we define visit efectiveness of the pollinator species *i* to plant species *j* as the proportion between the visits of *i* to *j* and all visits of *i*, multiplied by the proportion between the number of pollen grains of *j* attached to individuals of *i* and all pollen attached to individuals of *i*.

### Data collection

To obtain the number of shared pollinators for each species (*i*), we collected data to construct qualitative and quantitative polination networks. 
...
To obtain plant abundance (*ii*), we counted all units of floral atraction found during an independent sampling of the same transects used to construct the quantitative visitation networks. 
...
We investigate the impact of the ecological factors on the pollination services usingf two metrics: the quantity and the quality of pollination. 
Generally speaking, here we define the quatity of pollinaton as the amount of conspecific pollen and the quality of pollination as the amount of conspecific pollen *relative* to heterospecific pollen. 
To construct these metrics we use two sets of linear mixed models (LMM) in which the response variables are the number of conspecifc and heterospecific pollen grains deposited per stigma in flowers open to animal mediated pollination. 
...
Because the amount of deposited pollen can vary widely across species, and potentially also across communities, we evaluated two possible structures for the random effects: one that includes a random intercept for plant species, and one that also includes a random intercept for community. 
...
We calculated the number of shared pollinators for each plant species (*i*) by pooling data from the qualitative and quantitative pollination networks. 
This variable is strongly correlated to the number of animal species a plant interacts with (the species degree). 
To calculate the plants relative abbundance in their community (*ii*), we simply aggregated floral counts for each species. 
We then calculated the mean visit effectiveness (*iii*); as explained before, we define thevisit effectiveness of pollinator species *i* for plant species *j* as \[e_{ij} = \frac{v_{ij}}{v_i} \frac{p_{ji}}{p_j}\] where *v~ij~* is the observed number of visits by *i* to *j*, *p~ji~* is the number of pollen grains from *j* attached to *i*, *v~i~* is the total number of visits performed by *i*, and *p~j~* is the total number of grains carried by *j*.
...
However, because floral counts across months are not independent, we asigned them a weight of 1/5 such that phenology has an equal weight as other traits. 
All traits were scaled prior to calculating the centroid of the functional space and the species-specific functional coordinates were calculated using the R package `FD 1.0-12` [@laliberte_distance-based_2010; @laliberte_fd_2014]. 
To facilitate comparison across variables, we rescaled all predictors to have a zero mean and unitary variance.

To estimate the coefficients and to perform model selection and the associated uncertainity, we used a combination of multi-model inference and bootstrap resampling with one hundred replicates. 
First, we preformed model selection using the AICc and determined the likelihood of each candidate model (a particular combination of predicting variables) by calculating the median \(\Delta\)AICc within boostrap resamples. 
As we wanted model coefficients from more likely candidante models to carry more weight in our results, we sampled the coefficients for our variables proportional to the likelyhood of its candidate model. 
We "shrinked" the estimates coefficient of potentially unimportant variables by setting its estimate to 0 when the variable was not included in the model. 
...
Again, here we used bootstrap resampling to estimate the uncertainity arround these means. 

## Plant strategies

Or last objective is to tease appart the strategies that plants might use to reduce competition and, importantly, how the community context influences these strategies. 
...
Alternatively, if community plays a large role, we should be able to observe differences in the strategy a plant species across communities. 
...
We then compare these two metrics to those obtained with 99 Monte Carlo randomisations of plant strategies that replace the focal plant strategy by one of another species in its community.
