...
We found that the overall slope of the relationship was positive (Table SXX), but a large proportion of plants experienced a negative or no significant relationship or a between heterospecific and conspecific pollen (`r counts_facilitation_slope$np[1]`% and `r counts_facilitation_slope$np[2]`% respectively; Figure \@ref(fig:fig-trade-off-predictions)a). 
...
Furthermore, although there was wide variation in the slope of the relationship between heterospecific and conspecific pollen, plants, in almost all cases, had more conspecific than heterospecific pollen deposited on their stigmas (Figure \@ref(fig:fig-trade-off-predictions)b). 
...
We tested two random structures. 
...
However, the difference between the coefficient that relates visit effectiveness to heterospecific and conspecific pollen deposition suggests a negative, but marginal, relationship of this variable and our definition of the quality of the pollination service (Figure \@ref(fig:fig-average-qual-quant)c). 
...
Finally, the number of shared pollinators had a negative association with the quantity and quality of pollination, but these impacts were small when compared to all other ecological factors. 
...
We used a PCA of the analysed species to investigate whether plants strategy are similar across communities or whether they are flexible, and therefore able to adapt to the community context. 
The first two PCA components explained `r round(this_pca$eig[2, 3])`% of the total variance (Figure \@ref(fig:fig-pca)a). 
The first component was dominated by changes on the visit effectiveness and the relative abundance while the second component was dominated by the number of shared pollinators and the plant's functional originality. 
...
Our results suggest that the community context plays a central role at determining competition for pollination. 
...
Second, we found that plants are able to minimise the intensity of this competition using multiple strategies, however, conspecific and heterospecific pollen deposition are tightly coupled and threfore these strategies are likely to involve a trade-off between the quantity and quality of pollination. 
Third, we found that these strategies are strongly shaped by the community context, this is, the same plant species can adopt dramatically different strategies depending on the comminity it belongs to. 

\R{disc-facil-comp}Both of the tests used to identify the effect of animal-mediated pollination suggested the same outcome: that overall facilitation is only experienced by a minority of species in a community. 
Althouh a recent study already suggested a wide range of outcomes in multi-species context [@tur_evaluating_2016], the factors determining why some species experience more competition than others were not understood. 
Here we show that prevalence of neutral and competitive effects of animal-mediated pollination can be, at least partially, explained by the trade-offs that minimising competition involves.  
...
Adopting a strategy of high visitation increasses the rates of conspecific pollen deposition at the expense of a reduction in the quality of pollination even when the visitors carry a high proportion of conspecific pollen. 
...
Simultaneously, because visitation, pollen production and abundance are correlated, these trade-offs also suggest that to minimise competition for pollination plants must reach a fine balance between these factors to increasse conspecific pollen and reduce heterospecific pollen deposition. 
The relative wide range of in our model coefficients might be explained by the wide variation of the extent to which plant species are affected by pollen deposition. 
While some species experience a large reduction on seed set due to heterospecific pollen deposition, some others are relatively insensitive to heterospecific pollen but are greatly affected by low levels of conspecific pollen deposition [@campbell_mechanism_1985]. 
...
Two other strategies involved a much smaller trade-off than the abundance or visit efficacy. 
The two other strategies we analysed involved a smaller trade-off than the abundance or visit efficacy. 
Specifically, having high functional originality relative to other species in the community or sharing few pollinators was found to be benetitial to both the quantity and quality of pollination.
Interestingly, these two strategies also had lower explanatory power in the models of pollen deposition. 
This does not necessarily implies that these variables are unimportant to pollen deposition. 
Instead, it is also possible that the processes that underlay these strategies might be more complex than what our metrics are able to capture. 
...
Alternatively, other factors that we were unable to meassure, like pollinator behaviour and densities [@ye_competition_2014; @flanagan_effects_2011; @melissas_2019?] or the spatial context [@cariveau_spatially_2009], can also shape the way sharing pollinators affect the pollination service. 
...
Lit? Context?
On the one hand these two variables are more tighly related to the understanding of competition at the community scale than metrics of dominance. 
On the other, having distinct traits, for example trough phenology differentiation, has a positive impact on the pollination service only to the extent to which previous floral communities have promoted high pollinator densities [@ye_competition_2014]. 
In addition, some plants have been shown possess mechanisms that prevent heterospecific pollen deposition even when shared visitors carry pollen loads from diverse species. 
Similarly, other factors, not meassured in our study, like pollinator behaviour and densities [@ye_competition_2014; ref to Melissa's paper?] or the spatial context [@cariveau_spatially_2009] can also shape the way sharing pollinators affects the pollination service. 
