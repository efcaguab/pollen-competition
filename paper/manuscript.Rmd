...
Our models of pollen deposition had relatively high explanatory power. 
The conditional coefficient of determination R^2^~(c)~ [@nakagawa_general_2013] for the most parsimonious models (those with the lowest AICc) ranged between `r r2_values$r2c_min[r2_values$pollen_category == "conspecific_abs"]` and `r r2_values$r2c_max[r2_values$pollen_category == "conspecific_abs"]` (mean `r r2_values$r2c_mean[r2_values$pollen_category == "conspecific_abs"]`) for the models of conspecific pollen and between `r r2_values$r2c_min[r2_values$pollen_category == "heterospecific_abs"]` and `r r2_values$r2c_max[r2_values$pollen_category == "heterospecific_abs"]` (mean `r r2_values$r2c_mean[r2_values$pollen_category == "heterospecific_abs"]`) for the models of heterospecific pollen. 
...
By examining the AICc scores of a set of linear mixed effect models, we found that the visit efficay and the plants abbundance were the most important ecological variables predicting the amount of conspecific and heterospecific pollen deposited in the plant stigmas (Figure \@ref(fig:fig-average-qual-quant)a). 
The plants functional originality had an intermediate importance, while, surprisingly, the number of shared pollinators was comparatively less unimportant as it was only included in models with relatively large AICc values. 

```{r fig-average-qual-quant, fig.height= 2.29, fig.width=6.5, fig.cap="Mean relative effect of ecological variables on the quantity and quality of pollination. Results are shown for values calculated with both the absolute gain in pollen density (abs.) and the gain relative to selfing (rel.). Bootstrap confidence intervals of the mean (at the 0.05 level) are shown in square brackets."}
height1 <- 1
height2 <- 1.29
