...
Specifically, we investigate the effect of (*i*) the number of shared pollinators, (*ii*) the plants relative abundance, (*iii*) the plants functional originality, and (*iv*) the mean visit effectiveness---a metric that combines the share of pollen that a plant species is able to place on each of their floral visitors and the number of visits it receives from them. 
...
To obtain data about (*i*) the number of shared pollinators for each species we pooled data from quantitative and qualitative polination networks. 
...
To obtain(*ii*) plant abundance, we counted all units of floral atraction found during an independent sampling of the same transects used to construct the quantitative visitation networks. 
We also recorded (*iii*) morphological traits that relate to plant type (herb, shrub, climber), life cycle (annual, perennial), flower colouration, and whether the species is native in the study region. 
In addition we used data from the abundance counts 
Finally, to estimate the (*iv*) visit effectiveness, in addition to the visitation networks we need to construct pollen transfer networks. 
