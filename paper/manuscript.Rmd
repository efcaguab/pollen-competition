...
Abundant plant species might experience a dilution of available pollinators [@feinsinger_effects_1987; @feldman_when_2004] but might also receive more effective visits by capitalising on a larger share of both visits and the pollen carried by pollinators. 
...
Specifically, we hypothesize that competition, measured as the quantity of conspecific pollen deposition and its purity (amount relative to heterospecific pollen), should increase for plants that share many pollination partners.
<!-- DBS: Should we interchange amount and quantity so loosely like in the start of the sentence above? Also, it seems weird to call quantity of conspecific pollen a proxy for competition since it should be the inverse... -->
...
Third and finally, we examine how much the community context influences the way plants exploit these factors to minimise competition. 
...
<!-- DBS: I find the phrase "competition for pollination niche" to be very clunky. Can we do better? -->
...
On the contrary, if a species' strategy is flexible, we should expect plants to be able to occupy different niches in each community such that competition could ultimately be minimised regardless of the context. 
<!-- DBS: Why do we always say could and not would? -->
...
In this model, the number of conspecific pollen grains per flower was the response variable and the number of heterospecific grains the predictor variable (the number of stigmas sampled was added as an offset term). 
<!-- DBS: I have a technical question about this offset. Is it the raw count or the log of said count? -->
...
<!-- DBS: This is slightly ambiguous. Are communities or species the random effect? -->
Models were fitted using the function `glmer` from the R package `lme4 1.1-19` [@bates_fitting_2015]. 
A positive slope for a plant-community combination might indicate facilitation while a negative slope might indicate competition [@tur_evaluating_2016]. 
<!-- DBS: Everywhere else, we use definitive language about this interpretation. Here we temper it with 'might'. Why? -->
...
Our second objective was to investigate the relative contribution of different factors---that describe the plants' competition for pollination niches---to the pollination service. 
Generally speaking,
<!-- in the context of competition for pollination, -->
...
To obtain plant abundance, we counted all units of floral attraction found during an independent sampling of the same transects used to construct the quantitative visitation networks. 
...
To calculate the plants' relative abundance in their community, we aggregated floral counts for each species. 
...
<!-- DBS: Log transforming power-law distributed variables doesn't give them a natural scale (which is the overarching point of transforming variables) since they're scale free by definition... -->
...
To include phenological variation, we treated plant abundance in each of the survey months (November to March) as a "trait" in our analysis. 
...
<!-- DBS: I'm lost. Why did we scale and then rescale? This doesn't seem to make any sense. -->
...
Alternatively, if the community plays a large role and/or plants strategies are flexible, we should be able to observe differences in the realised niche a plant species occupies across communities. 
To test this, we first used a principal component analysis (PCA) of the four ecological variables. 
