...
* Use AICc instead of AIC (no change in results)
* To keep signature of phenology, absolute flower counts (log-transformed) are used instead of relative abundances
* Columns in the trait matrix for phenology are scaled so that it doesn't count 5 times as much as other traits
* Only modeling open and bagged pollen counts, using the bagged as a control. So not showing the results of the relative pollen count (open vs bagged flowers) anymore
* Introduced visit efficiency instead of share in the pollen pool as it is a better predictor. 
* Use a PCA of the explanatory variables to understand different plant strategies
...
Pollination systems play a disproportionally important role in food production and maintenance of global biodiversity [@bascompte_plant-animal_2007; @klein_importance_2007; @ollerton_how_2011]. 
...
Indeed, studies that are fundamental to our current knowledge of mutualism predict that an upper limit to biodiversity is achieved when the number of mutualistic partners is maximised in a community [@bastolla_architecture_2009]. 
And yet, fully connected natural pollination communities do not exist. 

This is so, because sharing a mutualistic partner also has costs [@waser_interspecific_1978]. 
...
The possible negative effects of competition for pollinators have been long recognised [@robertson_philosophy_1895] and widely documented both experimentally [@lewis_experimental_1961] and theoretically [@levin_competition_1970]. 
Multiple factors (like the number of shared pollinators, the plants' relative abundance, visitator fidelity, and traits) have been shown to modulate the strength of competition and in turn the fitness outcome [@bobisud_pollinator_1975; @mitchell_new_2009; @morales_interspecific_2008; @feinsinger_pollinationiscommunity_1987; @campbell_pollinator_1985]. 
It is currently clear that these neghative effects on fitness have the potential to drive the evolution of flower phenologies, morphologies and reproductive strategies, as well as driving ecological differentiation [@mitchell_new_2009; @caruso_competition_2000]. 
With few exceptions [@rathcke_interactions_1988; @lopezaraizamikel_impact_2007], these findings are based mostly on two-species systems.  However, competitive interactions between species pairs do not occur in isolation. 

Instead, plants often occur in communities in which multiple intertwined factors operate simultaneously and lead to emergent phenomena not observed at smaller scales. 
For instance, recent empirical evidence suggest that species with original traits generally have fewer interaction partners [@coux_linking_2016]. 
...
If evolutionary specialisation occur by changing traits to focus on fewer but better partners, we should expect a reduction of competition for pollination in species with original traits and an increasse in species with a large number of interaction partners. 
...
When we take into account the fact that for example a species' generalism and its relative abundance are likely to be positively associated [@sargent_role_2006], distinguishing the strategies that plants might adopt to minimise competition becomes even harder because coorelated factors might lead to competing costs.

Elucidating competition for pollination at the community scale and the plants strategies to minimise it has been elusive is the sheer amount of data necessary to dissentangle them. 
